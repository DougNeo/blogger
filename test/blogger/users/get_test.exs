defmodule Blogger.Users.GetTest do
  ...
  describe "all/0" do
    ...
    test "when there are no users, returns an empty array" do
      ...
      assert {:error, %Blogger.Error{message: "Usuários não existem", status: :not_found}} = response
    end
  end
  ...
  describe "by_id/1" do
    ...
    test "when id is not valid, error user not found" do
      ...
      assert {:error, %Blogger.Error{message: "Usuário não existe", status: :not_found}} = response
    end
  end
  ...
  describe "by_email/1" do
    ...
    test "when email is not valid, error user not found" do
      ...
      assert {:error, %Blogger.Error{message: "Campos inválidos", status: :bad_request}} = response
    end
  end
end
