defmodule BloggerWeb.Auth.ErrorHandler do
  ...
  def auth_error(conn, {error, _reason}, _opts) do
    case error do
      ...
      :unauthenticated -> Conn.send_resp(conn, 401, Jason.encode!(%{message: "Token não encontrado"}))
      :invalid_token -> Conn.send_resp(conn, 401, Jason.encode!(%{message: "Token expirado ou inválido"}))
